...
contract ThePosStaking is Ownable{
    using SafeERC20 for IERC20; // Wrappers around ERC20 operations that throw on failure

    ThePos public tposToken; // Token to be payed as reward
    ...
    // Staking pool -> Early stage just for stETH
    struct Pool {
    ...
    event UpdateRewardsPerBlock(uint256 rewardTokensPerBlock);

    // Constructor
    constructor(address _rewardTokenAddress, uint256 _rewardTokensPerBlock) {
        tposToken = ThePos(_rewardTokenAddress);
        rewardTokensPerBlock = _rewardTokensPerBlock;
    }

    /**
     * @dev Show reward amount per block 
     */
    ...
    /**
     * @dev Update reward amount per block -> should be discussed
     */
    function updateRewardsPerBlock(uint256 _rewardTokensPerBlock) external onlyOwner {
        rewardTokensPerBlock = _rewardTokensPerBlock;
        emit UpdateRewardsPerBlock(rewardTokensPerBlock);
    }
    ...
    function harvestRewards(uint256 _poolId) public {
        ...
        Pool storage pool = pools[_poolId];
        PoolStaker storage staker = poolStakers[_poolId][msg.sender];
        uint256 accumulatedRewards = staker.amount * pool.accumulatedRewardsPerShare / REWARDS_PRECISION;
        uint256 rewardsToHarvest = accumulatedRewards - staker.rewardDebt;
        if (rewardsToHarvest == 0) {
            staker.rewardDebt = accumulatedRewards;
            return;
        }
        staker.rewards = 0;
        staker.rewardDebt = accumulatedRewards;
        emit HarvestRewards(msg.sender, _poolId, rewardsToHarvest);
        tposToken.mint(msg.sender, rewardsToHarvest);
    }
